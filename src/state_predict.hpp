#ifndef __STATE_PREDICT__
#define __STATE_PREDICT__

// #include "time.hpp"
// #include "lib/include/ekf.h"
// #include <algorithm>
...
//     Function_H fun_h_;
//     Timer ekf_timer_;
//     Eigen::Matrix<double, 4, 1> estimate_result_;
//     Eigen::Matrix<double, 4, 1> predict_result_;

// public:
//     State();
//     State(Eigen::Matrix<double, 9, 9> Q, Eigen::Matrix<double, 4, 4> R);
//     ~State();
//     void init(Eigen::Matrix<double, 9, 9> Q, Eigen::Matrix<double, 4, 4> R);
//     Eigen::Matrix<double, 4, 1> estimate(const Eigen::Matrix<double, 4, 1>& current_pose, double );
//     Eigen::Matrix<double, 4, 1> predict(double delay);
// };
// }
...
namespace EKF
{
    ...
    public:
        State();
        State(double r_min, double r_max, int armor_num = 4);
        ~State() {};
        void default_set();
        Eigen::Matrix<double, 4, 1> estimate_pose(const Eigen::Matrix<double, 4, 1> &pose, const TimePoint &t, int toggle = 0);
        Eigen::Matrix<double, 4, 1> predict_pose(double delay);
        void setMatrixQ(Eigen::Matrix<double, 9, 9> Q);
        void setMatrixR(Eigen::Matrix<double, 4, 4> R);
    ...
    private:
        ...
        EKF<9, 4> model_;       // 运动模型
        Function_F fun_f_; // 运动模型的状态转移函数
        Function_H fun_h_; // 运动模型的观测函数
